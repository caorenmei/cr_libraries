...
namespace cr
{
    namespace network
    {
        ...
        const boost::asio::ip::tcp::endpoint& PbConnection::getLocalEndpoint() const
        {
            return localEndpoint_;
        }

        const boost::asio::ip::tcp::endpoint& PbConnection::getRemoteEndpoint() const
        {
        ...
        bool PbConnection::decodePacket()
        {
            ...
            while (inputBuffer_.size() >= sizeof(packetLength) + sizeof(checksum) + 1)
            {
                ...
                inputBuffer_.consume(packetLength);
                // 处理消息
                if (messageHandler_)
                {
