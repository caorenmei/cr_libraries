...
struct chunk_encoding_parser {
  ...
  std::enable_if_t<
    std::is_same<CharT, typename char_<Tag>::type>::value && 
    !std::is_same<CharT const *, typename std::array<typename char_<Tag>::type, 1024>::const_iterator>::value>
    update_chunk_size(boost::iterator_range<CharT const *> const &range) {
      update_chunk_size(make_iterator_range(
          typename std::array<typename char_<Tag>::type, 1024>::const_iterator(boost::begin(range)),
          typename std::array<typename char_<Tag>::type, 1024>::const_iterator(boost::end(range))));
  }
  ...
  std::enable_if_t<
      std::is_same<CharT, typename char_<Tag>::type>::value &&
      !std::is_same<CharT const *, typename std::array<typename char_<Tag>::type, 1024>::const_iterator>::value,
      boost::iterator_range<typename std::array<typename char_<Tag>::type, 1024>::const_iterator>>
      operator()(boost::iterator_range<CharT const *> const &range) {
      return operator()(make_iterator_range(
              typename std::array<typename char_<Tag>::type, 1024>::const_iterator(boost::begin(range)),
              typename std::array<typename char_<Tag>::type, 1024>::const_iterator(boost::end(range))));
  }
};
